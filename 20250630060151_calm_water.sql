...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🔄 Processing expired sessions at:', new Date().toISOString())

    // Find sessions that have expired but haven't been processed
    ...
    if (sessionsError) {
      throw new Error(`Failed to fetch expired sessions: ${sessionsError.message}`)
    }

    console.log(`📊 Found ${expiredSessions?.length || 0} expired sessions to process`)

    const processedSessions = []

    for (const session of expiredSessions || []) {
      try {
        console.log(`⚙️ Processing session: ${session.id} - ${session.title}`)

        // Mark session as inactive and report as generated
        ...
        // If there are responses, generate and send report
        if (session.response_count > 0) {
          console.log(`📈 Generating report for session ${session.id} with ${session.response_count} responses`)
          ...
          if (responsesError) {
            console.error(`❌ Failed to fetch responses for session ${session.id}:`, responsesError)
            continue
          }
          ...
          if (overallPercentage >= 80) performanceCategory = 'excellent';
          else if (overallPercentage >= 60) performanceCategory = 'good';
          else if (overallPercentage >= 40) performanceCategory = 'fair';
...
<body>
    ...
    <div class="question-block">
      ...
      <div class="question-stats">
        <div class="stat-item">
          ...
          <div class="stat-value">${(question_averages[index] || 0).toFixed(2)}/${scaleMax}</div>
        </div>
        <div class="stat-item">
          <div class="stat-label">Median Score</div>
          <div class="stat-value">${(question_medians[index] || 0).toFixed(1)}</div>
        </div>
      </div>
      
      <div class="distribution">
        ${(response_distribution[index] || []).map((count, i) => `
  ...
  <div class="footer">
      ...
      This report contains confidential leadership feedback. Please handle with appropriate discretion.
          ...
          try {
            ...
            if (notificationError) {
              console.error(`⚠️ Notification error for session ${session.id}:`, notificationError)
            } else {
              console.log(`📧 Report notification sent for session ${session.id}`)
            }
            ...
            console.log(`✅ Report processing completed for session ${session.id}`)
          } catch (notificationError) {
            console.error(`❌ Failed to send report for session ${session.id}:`, notificationError)
          }
        } else {
          console.log(`📭 Session ${session.id} had no responses, sending no-response notification`)
          
          // Send notification about session with no responses
          try {
            ...
            // Mark as processed even with no responses
            await supabase
              .from('feedback_sessions')
              .update({
                report_sent: true,
            ...
            console.log(`📧 No-response notification sent for session ${session.id}`)
          } catch (notificationError) {
            console.error(`❌ Failed to send no-responses notification for session ${session.id}:`, notificationError)
          }
        }
    ...
    // Also notify admin about the daily processing summary
    if (processedSessions.length > 0) {
      try {
        await supabase.functions.invoke('send-notification', {
          body: {
            type: 'daily_processing_summary',
            processed_sessions: processedSessions,
            email: 'chloe@cultivatedhq.com.au'
          }
        })
      } catch (summaryError) {
        console.error('Failed to send daily summary:', summaryError)
      }
    }

    const successCount = processedSessions.filter(s => s.status === 'processed').length
    const errorCount = processedSessions.filter(s => s.status === 'error').length
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: `Processed ${processedSessions.length} expired sessions`,
        summary: {
