...
serve(async (req) => {
  ...
  try {
    ...
    // Get request data
    const { auditId, email } = await req.json();

    console.log(`🔍 Processing clarity audit report for: ${email || auditId}`);
    ...
    if (auditId) {
      ...
      if (error) throw new Error(`Failed to fetch audit result: ${error.message}`);
      auditResult = data;
    } else if (email) {
      const { data, error } = await supabase
        .from("clarity_audit_results")
      ...
      throw new Error("Either auditId or email must be provided");
    }

    if (!auditResult) {
      throw new Error("No audit result found");
    }
    ...
    console.log("🔄 Calling PDF.co API to generate PDF...");
    const pdfResponse = await fetch("https://api.pdf.co/v1/pdf/convert/from/html", {
      ...
      body: JSON.stringify({
        html: htmlReport,
        name: `Leadership-Clarity-Audit-${auditResult.email.replace(/[^a-z0-9]/gi, "_")}.pdf`,
        async: false,
        margins: "20px",
        paperSize: "A4",
    ...
    const emailBody = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background: #f5f5f0; padding: 20px; border-radius: 10px; margin: 20px 0; border-left: 4px solid #2a9d8f;">
          ...
          <ul>
            <li><strong>Overall Score:</strong> ${auditResult.total_score.toFixed(1)}/5</li>
            <li><strong>Strongest Area:</strong> ${formatCategoryName(auditResult.highest_category)}</li>
            <li><strong>Development Area:</strong> ${formatCategoryName(auditResult.lowest_category)}</li>
          </ul>
        </div>

        <div style="text-align: center; margin: 30px 0;">
          <a href="${pdfResult.url}" style="background: #2a9d8f; color: white; padding: 15px 30px; text-decoration: none; border-radius: 25px; font-weight: bold;">
        ...
        <p style="font-size: 12px; color: #666;">
          This email was sent because you completed the Leadership Clarity Audit at Cultivated HQ.
        </p>
      </div>
    `;
    ...
    console.log(`📧 Email would be sent to: ${auditResult.email}`);
    console.log(`📧 Subject: ${emailSubject}`);
    console.log(`📧 PDF URL: ${pdfResult.url}`);

    // Update the audit result with the PDF URL
    ...
    if (updateError) {
      console.error(`⚠️ Failed to update audit result with PDF URL: ${updateError.message}`);
    }
